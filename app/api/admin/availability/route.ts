...
export async function PATCH(request: Request) {
  ...
  const { data: reservations } = await supabaseAdmin
    .from("reservations")
    .select(
      "id, guest_name, guest_email, guest_phone, status, start_date, end_date",
    )
    .eq("property_id", propertyId)
    .in("status", ["pending", "confirmed"])
    .lte("start_date", endDate)
  ...
  const reservationByDate = new Map<
    string,
    NonNullable<typeof reservations>[number]
  >();
  reservations?.forEach((reservation) => {
    let date = reservation.start_date;
    while (date < reservation.end_date) {
      if (!reservationByDate.has(date))
        reservationByDate.set(date, reservation);
      date = nextDate(date);
    }
  });
  ...
  while (current <= endDate) {
    ...
    if (available !== undefined)
      update.status = available ? "available" : "blocked";
